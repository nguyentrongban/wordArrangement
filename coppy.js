...
// Ghi lại lượt truy cập và lấy tổng số lượt truy cập mỗi khi trang được tải
window.onload = async function() {
    try {
        await logVisit();
        await getTotalVisits();
        startTimer(); // Bắt đầu đếm thời gian khi trang được tải
    } catch (error) {
...
let timeoutID;
function startTimer() {
    timeoutID = setTimeout(showHint, 90000); // 90000 milliseconds = 1.5 phút
}
